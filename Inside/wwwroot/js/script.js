...
class PixelDisplay240App extends EditorModule {
    ...
    _export() {
        ...
        // C Array Export Logic (RGB565, RGB888, RLE)
        ...
        let code = `// Gerado pelo PixelDisplay240 IDE v5\n`;
        code += `// Nome: ${arrayName} (${width}x${height})\n`;
        code += `// Formato: ${format.toUpperCase()}\n\n`;
        code += `#include <pgmspace.h>\n\n`;
        ...
        if (format === 'rgb565') {
            ...
            for (let i = 0; i < imgData.length; i += 4) {
                ...
                code += `0x${hex.toString(16).toUpperCase().padStart(6, '0')}`;
                if (i < imgData.length - 4) code += ', ';
                if (++count >= 8) { code += '\n  '; count = 0; }
            }
            code += `\n};\n`;
        ...
        link.click();
        URL.revokeObjectURL(link.href);
        this.toast.show('success', 'Exportação Concluída', `Arquivo ${arrayName}.h gerado!`);
    }
...
class LogManager extends EditorModule {
    ...
    error(context, message, data = {}) {
        ...
        localStorage.setItem('pixeldisplay240_logs', JSON.stringify(this.logs));

...
class LayerManager extends EditorModule {
    ...
    add(name = "Camada", dataURL = null, silent = false) {
        ...
        // Update rulers and grid after adding a layer
        if (this.app.layout) {
            this.app.layout.updateRulers();
            this.app.layout.renderGrid();
        }
...
constructor(app) {
    ...
    this.canvasWrapper = this.dom('canvas-wrapper');

    // Set initial canvas wrapper size
    this._initCanvasSize();

    this._setupEvents();

    // Defer initial render to allow layers to be created first
    setTimeout(() => {
        this.updateRulers();
        this.renderGrid();
    }, 100);
...
_initCanvasSize() {
    ...
    const tempCanvas = this.dom('temp-canvas');
    if (tempCanvas) {
        tempCanvas.width = w;
        tempCanvas.height = h;
    }
    ...
    updateRulers() {
        ...
        // Use active layer dimensions or fallback to configured canvas size
        const active = this.app.layers?.active;
        const w = active ? active.canvas.width : (parseInt(this.dom('canvas-width')?.value) || 240);
        const h = active ? active.canvas.height : (parseInt(this.dom('canvas-height')?.value) || 240);

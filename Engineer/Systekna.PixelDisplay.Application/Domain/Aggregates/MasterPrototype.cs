...
namespace Systekna.PixelDisplay.Application.Domain.Aggregates;
...
public class MasterPrototype
{
    ...
    public void AddAsset(PrototypeAsset asset)
    {
        ...
        var safeName = Regex.Replace(asset.Name, "[^a-zA-Z0-9_]+", "_");
    ...
    public bool RemoveAsset(string name)
    {
        ...
        foreach (var screen in _project.Screens)
        {
            ...
            foreach (var el in screen.Elements)
            {
                if (el.Asset == name) el.Asset = null;
            }
        }

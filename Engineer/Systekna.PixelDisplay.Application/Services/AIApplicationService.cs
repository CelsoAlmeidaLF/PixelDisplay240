...
/// <summary>
/// Serviço de IA para geração de imagens e otimização de layout.
/// </summary>
public class AIApplicationService : IAIService
{
    ...
    public async Task<GenerateImageResult> GeneratePixelArtAsync(string apiKey, string prompt, int seed)
    {
        if (string.IsNullOrWhiteSpace(apiKey))
        {
            return new GenerateImageResult(false, null, "Gemini API key not configured");
        }
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            return new GenerateImageResult(false, null, errorBody);
        }
        ...
        try
        {
            ...
            {
                return new GenerateImageResult(true, bytes, null);
            }

            Console.WriteLine("[AI] No image bytes found in response:");
            Console.WriteLine(responseText);
            return new GenerateImageResult(false, null, "No image bytes found");
        }
        catch (Exception ex)
        {
            Console.WriteLine($"[AI] Failed to parse response: {ex.Message}");
            Console.WriteLine(responseText);
            return new GenerateImageResult(false, null, "Failed to parse response");
        }
    }

    public async Task<OptimizeLayoutResult> OptimizeLayoutAsync(string apiKey, string elementsJson, string screenIntent)
    {
        if (string.IsNullOrWhiteSpace(apiKey))
        {
            return new OptimizeLayoutResult(false, null, "Gemini API key not configured");
        }
        ...
        try
        {
            ...
            {
                var errorBody = await response.Content.ReadAsStringAsync();
                return new OptimizeLayoutResult(false, null, $"Gemini API Error: {errorBody}");
            }
            ...
            return new OptimizeLayoutResult(true, resultText.Trim(), null);
        }
        catch (Exception ex)
        {
            return new OptimizeLayoutResult(false, null, ex.Message);
        }
    }
    ...
    #region Image Extraction Helpers
